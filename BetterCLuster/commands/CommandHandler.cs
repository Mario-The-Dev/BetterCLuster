...
using BetterCLuster.application.multiShell;
using BetterCLuster.application.ClusterPasswords;
using BetterCLuster.Commands.ClusterExplorer;
using BetterCLuster.Commands.PyPlugsSys;
        ...
        public CommandHandler()
        {
            try
            {
                commands = new Dictionary<string, ICommand>
                {
                    ...
                    { "::info", new Info() },
                    { "now", new DateAndTime() },
                    { "color -all", new ChangeColorAll() },
                    { "color -font", new ChangeColorFont() },
                    ...
                    {"test//dev",new Experiment()},
                    {"shr -run",new shortRun_Run()},
                    {"shr -create",new shortRun_Create()},
                    {"visel",new Visel()},
                    {"pswrd",new PswrdsCommand()},
                    {"exp --beta",new Experi()},
                    {"pyplug",new PyPlugs()},     //[STATUS UNDER DEVELOPMENT & ]
                    //{"mS -cmd",new msCmd()},
                    //{"mS -ps",new msPowerShell()},
                    //{"",new ()},
